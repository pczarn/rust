...
pub fn int_to_str_bytes_common<T: Int>(num: T, radix: uint, sign: SignFormat, f: |u8|) {
    use ptr::RawPtr;
    use cast::transmute;
    static LOOKUP: &'static str = "zyxwvutsrqponmlkjihgfedcba9876543210123456789abcdefghijklmnopqrstuvwxyz";
    let (lookup, _): (*u8, uint) = unsafe { transmute(LOOKUP) };
    let lookup = unsafe { lookup.offset(35) };
